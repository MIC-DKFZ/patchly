...
class TestSqueezeSampler(unittest.TestCase):
    ...
    def test_with_offset_without_remainder_2d(self):
        ...
        self._test_sampler(image, spatial_size, patch_size, patch_offset)
    ...
    def test_patch_size_larger_than_spatial_size(self):
        ...
        self.assertRaises(RuntimeError, GridSampler, image=image, spatial_size=spatial_size, patch_size=patch_size, mode=SamplingMode.SAMPLE_SQUEEZE)
    ...
    def test_offset_size_larger_than_patch_size(self):
        ...
        self.assertRaises(RuntimeError, GridSampler, image=image, spatial_size=spatial_size, patch_size=patch_size, patch_offset=patch_offset, mode=SamplingMode.SAMPLE_SQUEEZE)
    ...
    def _test_sampler(self, image, spatial_size, patch_size, patch_offset=None, spatial_first=True):
        ...
        sampler = GridSampler(image=image, spatial_size=spatial_size, patch_size=patch_size, patch_offset=patch_offset, spatial_first=spatial_first, mode=SamplingMode.SAMPLE_SQUEEZE)
        ...
        for patch, patch_bbox in sampler:
            if not spatial_first:
                ...
                _patch_size = patch.shape[:len(patch_size)]
            if not np.array_equal(_patch_size, patch_size):
                print("")
        ...
        # Test without image
        result = np.zeros_like(image)
        sampler = GridSampler(spatial_size=spatial_size, patch_size=patch_size, patch_offset=patch_offset, spatial_first=spatial_first, mode=SamplingMode.SAMPLE_SQUEEZE)
